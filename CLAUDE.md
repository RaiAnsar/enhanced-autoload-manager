...
The **repository root is the canonical source of truth** for the plugin. The
`enhanced-autoload-manager-svn/trunk/` copy is synced from root for WordPress.org
deployment only (see MAINTENANCE.md). `wordpress/` is a local WP install for
testing and is git-ignored.

```
.
├── enhanced-autoload-manager.php   # Main plugin file (canonical)
...
├── assets-source/                  # Source SVGs for the assets
├── credentials.md                  # SVN credentials (git-ignored — DO NOT COMMIT)
├── CLAUDE.md                       # This file
├── MAINTENANCE.md                  # Release/deploy guide
├── enhanced-autoload-manager-svn/  # SVN deploy checkout (.svn git-ignored)
└── wordpress/                      # Local WP install for testing (git-ignored)
```
